...
enum labels, judge ids, and exception TYPE names — never report bytes and
never raw judge output (a coerced judge's freeform text is attacker-shaped;
it is parsed to an enum and discarded).
...
async def _ask_anthropic(judge: dict, sealed: str) -> JudgeVote:
    ...
    except Exception as e:  # noqa: BLE001 — exception TYPE only (Invariant 4)
        return JudgeVote(jid, "unavailable", f"api-error:{type(e).__name__}")
    try:
        text = " ".join(
