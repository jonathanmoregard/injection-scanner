...
Two phases:
  ...
  2. Honeypot liveness — one benign payload with use_honeypot=True.
     Pays one round-trip per call. If L3 is degraded (SDK missing,
     key revoked, provider down), production scans fail-closed per
     intercept.py; this probe surfaces that BEFORE any real record
     is processed.
...
from injection_scanner.intercept import Verdict, scan_text
...
def run_smoke(
    *,
    ...
    for c in _DETERMINISTIC:
        v: Verdict = scan_text(c.payload, use_honeypot=False)
        if v.ok:
            failures.append(f"{c.label}: payload passed (expected block)")
            continue
        if c.expected_reason_substring not in v.reason:
            failures.append(
                f"{c.label}: blocked but wrong layer "
    ...
    # Honeypot liveness: a benign payload must produce a "pass" honeypot
    # layer. Anything else (skipped, unavailable, accidentally triggered)
    # means infra rot or false-positive regression.
    v = scan_text(_BENIGN_PROBE, use_honeypot=True)
    if not v.ok:
        msg = (
    ...
    info(
        f"scanner self-test OK ({len(_DETERMINISTIC)} deterministic "
        "canaries blocked; honeypot live)"
    )
